...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Instrument+Sans:wght@400;500;600;700&display=swap'); * { box-sizing:border-box; } ::placeholder { color:#9ca3af; } ::-webkit-scrollbar { width:5px; } ::-webkit-scrollbar-thumb { background:#e5e7eb; border-radius:3px; } @keyframes fadeUp { from{opacity:0;transform:translateY(6px)} to{opacity:1;transform:translateY(0)} } @keyframes spin { to{transform:rotate(360deg)} } @keyframes pulse { 0%,100%{opacity:0.4} 50%{opacity:1} } @keyframes logospin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} } .chip:hover { background:#eff6ff !important; border-color:#bfdbfe !important; color:#1d4ed8 !important; } .go:hover { background:#0d3d6b !important; } .hist:hover { background:#f3f4f6 !important; } .new-chat:hover { background:#e5e7eb !important; } textarea { resize:none; } @media (max-width: 768px) { .sidebar { transform: translateX(-100%) !important; } .sidebar.open { transform: translateX(0) !important; } .main-content { margin-left: 0 !important; } .input-bar { left: 0 !important; } .two-col { grid-template-columns: 1fr !important; } .sidebar-overlay { display: block !important; } .chip { font-size: 11px !important; padding: 5px 10px !important; } h1 { font-size: 20px !important; } }`}</style>
